...
DESCRIPTION
-----------
srcom is a compositor based on Dana Jansens' version of xcompmgr (which itself was written by Keith Packard). It includes some improvements over the original xcompmgr, like window frame opacity and inactive window transparency.
...
*--config* _PATH_::
	Look for configuration file at the path. See xref:_configuration_files[*CONFIGURATION FILES*] section below for where srcom looks for a configuration file by default. Use `/dev/null` to avoid loading configuration file.

*--write-pid-path* _PATH_::
	Write process ID to a file. It is recommended to use an absolute path.

*--plugins* _PATH_::
	Specify plugins to load. Plugins will first be searched in current working directory (unless specified in the config file, in which case this step is skipped), then in `$XDG_CONFIG_HOME/srcom/plugins`, then in `$XDG_CONFIG_DIRS/srcom/plugins`. If all of the above fail, the plugin name is passed directly to the dynamic loader. Can be specified multiple times to load more than one plugins.
...
*--xrender-sync-fence*::
	Use X Sync fence to sync clients' draw calls, to make sure all draw calls are finished before srcom starts drawing. Needed on nvidia-drivers with GLX backend for some users.
...
+
NOTE: Setting _opacity_ explicitly with a rule will override the opacity value from the window properties (i.e. _pass:[_]NET_WM_WINDOW_OPACITY_), which is used by tools like `srcom-trans` for setting the opacity of window. If you would like to keep using tools like `srcom-trans`, you can choose to set the opacity only for windows without the opacity property by matching `! _NET_WM_WINDOW_OPACITY`.
...
srcom supports xref:fading[fading] animation when you open or close a window. In addition to that, srcom also has a very powerful animation script system, which can be used to animate many aspects of a window based on certain triggers. Animation scripts can be defined in your configuration file by setting the option _animations_. It is also possible to define animations per-window using the xref:_window_rules[*WINDOW RULES*] system, by setting the <<window-rules-animations>> option in a rule. (Read the rest of this section first before you go there.)
...
Defining an animation is a bit involved. To make animations more approachable, without you having to learn the nitty-gritty details of the script system, srcom provides a number of presets that you can use by just specifying a handful of options.
...
ifndef::env-web[]
NOTE: Describing animations with only words is difficult. We have short video clips showing off each preset, but sadly they cannot be included in this manpage. The web version of this document hosted on our website at https://srcom.app[] on the other hand, does have those clips.
endif::[]
...
A context variable is a variable srcom defines for you, and you can use them in expressions like any other variables. Their values reflect certain attributes of the window you are animating.
...
If you have created an animation script that you think is particularly cool, you are encouraged to share it with the community. You can submit an issue or a pull request to srcom on GitHub, and get a chance to have your animation included as one of the presets, so it can be used by everyone.

SHADER INTERFACE
----------------

This section describes the interface of a custom shader, how it is used by srcom, and what parameters are passed by srcom to the shader.

A custom shader is a GLSL fragment shader program, which can be used to override the default way of how a window is rendered. If a custom shader is used, the default srcom effects (e.g. dimming, color inversion, etc.) will no longer be automatically applied. It would be the custom shader's responsibility to apply these effects.

The interface between srcom and a custom shader is dependent on which backend is being used. The xrender backend doesn't support shader at all. Here we describe the interface provided by the glx backend.

The shader must define a function, _vec4 window_shader()_, which would be the entry point of the shader. The returned _vec4_ will be used to set __gl_FragColor__. A function, _vec4 default_post_processing(vec4 c)_, is provided for applying the default srcom effects to input color 'c'.
...
The default behavior of srcom window rendering can be replicated by the following shader:
...
CONFIGURATION FILES
-------------------
srcom could read from a configuration file if libconfig support is compiled in. If *--config* is not used, srcom will seek for a configuration file in `$XDG_CONFIG_HOME/srcom.conf` (`~/.config/srcom.conf`, usually), then `$XDG_CONFIG_HOME/srcom/srcom.conf`, then `$XDG_CONFIG_DIRS/srcom.conf` (often `/etc/xdg/srcom.conf`), then `$XDG_CONFIG_DIRS/srcom/srcom.conf`.

When `@include` directive is used in the config file, srcom will first search for the included file in the parent directory of `srcom.conf`, then in `$XDG_CONFIG_HOME/srcom/include/`, then in `$XDG_CONFIG_DIRS/srcom/include`.

srcom uses general libconfig configuration file format. A sample configuration file is available as `srcom.sample.conf` in the source tree. Most of command line switches can be used as options in configuration file as well. For example, *--vsync* option documented above can be set in the configuration file using `vsync = `. Command line options will always overwrite the settings in the configuration file.
...
* srcom reinitializes itself upon receiving `SIGUSR1`.

D-BUS API
---------

It's possible to control srcom via D-Bus messages, by running srcom with *--dbus* and send messages to `com.github.chjj.compton.<DISPLAY>`. `<DISPLAY>` is the display used by srcom, with all non-alphanumeric characters transformed to underscores. For `DISPLAY=:0.0` you should use `com.github.chjj.compton._0_0`, for example.
...
+
------------
$ srcom --config /dev/null
------------

* Run srcom with client-side shadow and fading:
+
------------
$ srcom -cf
------------
...
+
------------
$ srcom -bcf -i 0.8 -e 0.8 --no-fading-openclose
------------

* Draw white shadows:
+
------------
$ srcom -c --shadow-red 1 --shadow-green 1 --shadow-blue 1
------------

* Avoid drawing shadows on wbar window:
+
------------
$ srcom -c --shadow-exclude 'class_g = "wbar"'
------------

* Enable VSync with GLX backend:
+
------------
$ srcom --backend glx --vsync
------------

BUGS
----
Please submit bug reports to <https://github.com/yshui/srcom>.
...
SEE ALSO
--------
*xcompmgr*(1), xref:srcom-inspect.1.adoc[*srcom-inspect*(1)], xref:srcom-trans.1.adoc[*srcom-trans*(1)]
